...
export function GenerateForProject({
  ...
  action: (projectId: string) => Promise<{ ok: boolean; reason?: string }>;
}) {
  ...
  const go = () => {
    ...
    setMsg("");
    start(async () => {
      const r = await action(projectId);
      if (r.ok) { setOpen(false); router.refresh(); }
      else setMsg(r.reason === "exists" ? "Already generated for this project." : "Could not generate.");
    });
  };
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-ink/30 p-4 backdrop-blur-sm" onClick={() => !pending && setOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="mt-24 w-full max-w-md rounded-[var(--radius-lg)] border border-line bg-surface shadow-[var(--shadow-pop)]">
            ...
            <div className="p-5">
              <label className="text-[11.5px] font-semibold text-ink-2">Project</label>
              ...
              {msg && <p className="mt-2 text-[12px] font-medium text-warn">{msg}</p>}
            </div>
            <div className="flex justify-end gap-2 border-t border-line px-5 py-3.5">
              ...
              <button onClick={go} disabled={pending || !projectId} className="rounded-lg bg-brand px-4 py-2 text-[13px] font-semibold text-white shadow-[var(--shadow-rail)] hover:bg-brand-ink disabled:opacity-50">
