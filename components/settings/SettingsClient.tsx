...
import { TENANT, USERS } from "@/lib/mock/org";
import { FAMILIES } from "@/lib/configurator/families";
import { attrCount, RATES, RATE_CARD_DATE } from "@/lib/configurator/model";
import { WORKFLOWS, STANDARDS, RATE_CARD_HISTORY, BRAND_COLORS } from "@/lib/mock/settings";
...
export function SettingsClient() {
  ...
  const [families, setFamilies] = useState(() => FAMILIES.map((f) => ({ id: f.id, active: true })));
  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[210px_1fr]">
      ...
      <div className="min-w-0">
        {tab === "org" && <Org brand={brand} setBrand={setBrand} />}
        {tab === "families" && <Families families={families} setFamilies={setFamilies} />}
...
/* ---------- Organization ---------- */
function Org({ brand, setBrand }: { brand: string; setBrand: (c: string) => void }) {
  return (
    <Panel title="Organization" desc="These settings white-label the entire platform for this tenant.">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <SelectField label="Primary currency" value={TENANT.primaryCurrency} options={["INR", "USD", "EUR"]} />
...
/* ---------- Product Families ---------- */
function Families({ families, setFamilies }: { families: { id: string; active: boolean }[]; setFamilies: (f: { id: string; active: boolean }[]) => void }) {
  const toggle = (id: string) => setFamilies(families.map((f) => (f.id === id ? { ...f, active: !f.active } : f)));
  return (
    <Panel title="Product Families" desc="Families are defined as data — adding a product line is configuration, not code."
      action={<button className="flex items-center gap-1.5 rounded-lg bg-brand px-3 py-1.5 text-[12.5px] font-semibold text-white hover:bg-brand-ink"><Plus className="h-4 w-4" /> New family</button>}>
      <div className="divide-y divide-line-2 rounded-lg border border-line">
        {FAMILIES.map((f) => {
          const state = families.find((x) => x.id === f.id)!;
          return (
            <div key={f.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11.5px] text-ink-3">{f.category} · {attrCount(f)} attributes · {f.blurb}</div>
              </div>
              <Link href="/configurator" className="text-[12px] font-semibold text-brand hover:underline">Configure →</Link>
              <Toggle on={state.active} onClick={() => toggle(f.id)} />
            </div>
          );
        })}
      </div>
    ...
    <Panel title="Rate Cards" desc="Rate cards are dated — an old quotation always reproduces its original cost.">
      ...
      <div className="overflow-hidden rounded-lg border border-line">
        <div className="flex items-center justify-between border-b border-line-2 bg-surface-2 px-4 py-2 text-[11px] font-bold uppercase tracking-wide text-ink-4">
          <span>Current rate card · effective {shortDate(RATE_CARD_DATE)}</span><span>Rate</span>
        </div>
...
function Standards() {
  return (
    <Panel title="Standards Library" desc="Global standards are shared across all tenants; tenant standards are yours alone."
      action={<button className="flex items-center gap-1.5 rounded-lg border border-line bg-surface px-3 py-1.5 text-[12.5px] font-semibold text-ink-2 hover:bg-surface-3"><Plus className="h-4 w-4" /> Add</button>}>
      <div className="divide-y divide-line-2 rounded-lg border border-line">
        {STANDARDS.map((s) => (
...
function Team() {
  return (
    <Panel title="Team & Roles" desc="Members and their departments. Roles drive the approval chains above."
      action={<button className="flex items-center gap-1.5 rounded-lg bg-brand px-3 py-1.5 text-[12.5px] font-semibold text-white hover:bg-brand-ink"><Plus className="h-4 w-4" /> Invite</button>}>
      <div className="divide-y divide-line-2 rounded-lg border border-line">
        {USERS.map((u) => (
          <div key={u.id} className="flex items-center gap-3 px-4 py-2.5">
            ...
            <div className="hidden text-right sm:block">
              ...
              <div className="text-[11px] text-ink-4">{DEPARTMENT_LABEL[u.department]}</div>
            </div>
          </div>
        ))}
      </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <input defaultValue={value} className="mt-1 w-full rounded-lg border border-line bg-surface px-3 py-2 text-[13px] text-ink outline-none focus:border-brand-line focus:ring-2 focus:ring-brand-soft" />
    </div>
  );
}
function SelectField({ label, value, options }: { label: string; value: string; options: string[] }) {
  return (
    <div>
      <FieldLabel>{label}</FieldLabel>
      <select defaultValue={value} className="mt-1 w-full rounded-lg border border-line bg-surface px-3 py-2 text-[13px] text-ink outline-none focus:border-brand-line">
...
function Toggle({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={cn("relative h-5 w-9 shrink-0 rounded-full transition-colors", on ? "bg-brand" : "bg-line-strong")}>
...
function SaveBar() {
  return (
    <div className="mt-5 flex justify-end gap-2 border-t border-line-2 pt-4">
      <button className="rounded-lg border border-line bg-surface px-4 py-2 text-[13px] font-semibold text-ink-2 hover:bg-surface-3">Cancel</button>
      <button className="rounded-lg bg-brand px-4 py-2 text-[13px] font-semibold text-white shadow-[var(--shadow-rail)] hover:bg-brand-ink">Save changes</button>
    </div>
  );
}
